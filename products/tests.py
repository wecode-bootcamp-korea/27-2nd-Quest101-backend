...
class OrderViewTest(TestCase):
    def setUp(self):
        ...
        UserCourseStat.objects.bulk_create([
            UserCourseStat(id = 1,
                           user_id = 1,
                           course_stat_id = 1)
        ])
    ...
    def test_orderview_course_stat_does_not_exists(self):
        client = Client()
        response = client.post('/products/3/order', **headers)
        self.assertEqual(response.json(),
            {"message" : "INVAILD_COURSE_STAT"})    
